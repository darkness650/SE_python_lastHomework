    ...
    def load_reference_video(self, file_path: str) -> bool:
        ...
        try:
            ...
            self.motion_counter = TemplateRepetitionCounter(
                self.ref_template,
                config,
                key_actions=self.settings["target_count"]
                if self.settings["target_count"] > 0
                else None,
            )
        ...
        try:
            ...
            if landmarks and landmarks.data is not None:
                angles = compute_joint_angles(
                    landmarks.data, visibility_threshold=self.settings["visibility_threshold"]
                )

            # 准备标注信息
            annotations = {"count": current_count, **{f"{k}_angle": v for k, v in angles.items()}}
            ...
            self.count_updated.emit(current_count, self.settings["target_count"], rate)
