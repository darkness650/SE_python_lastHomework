        ...
        try:
            if len(frame.shape) == 3:
                ...
                if channels == 3:
                    ...
                    q_image = QImage(
                        rgb_frame.data, width, height, width * channels, QImage.Format.Format_RGB888
                    )
                else:
                    q_image = QImage(
                        frame.data, width, height, width * channels, QImage.Format.Format_ARGB32
                    )
            else:
                height, width = frame.shape
                q_image = QImage(frame.data, width, height, width, QImage.Format.Format_Grayscale8)
